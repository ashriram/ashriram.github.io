...
function bibtex2html($entry, $type, $accents, $hightlightName = '', $authorLimit = 0){
  ...
  if(trim($ret)==""){
    ...
    $webpdf = extractBib("pdf", $entry, $accents);
    $webppt = extractBib("ppt", $entry, $accents);
    $webgit = extractBib("webgit", $entry, $accents);
    ...
    if(trim($webpdf)!="") {
      $title = '<a href="'.$webpdf.'"> <b> '.$title.' </b> </a>';
    }
    // Ttitle:
    $ret .= '<span class="title">'.$title.'</span>. ';

     // Authors:
    $ret = $ret . '<br><span class="authors">'.formatAuthors($author, $hightlightName, $authorLimit).'</span> ';
    ...
    switch($type) {
      ...
      case "incollection":
        ...
        if(trim($booktitle)!=""){
          ...
          // Build output
          $ret = $ret.'<br> <span class="in">In '.$booktitle.'</span>';
          ...
          if(trim($series)    != "") $ret = $ret.', <b>('.$series.')</b>'.$volume.'';
          if(trim($year)    != "") $ret = $ret.", ".$year;
          $ret = $ret.".";
        }
        break;
    ...
    if(trim($webpdf) != "") {
      $ret .= '<br> <span class="webpdf"><a href="'.$webpdf.'" class="button pdf ugdv_link" target="_blank" >pdf</a></span>&nbsp;';
    }

    if(trim($webppt) != "") {
      $ret .= ' <span class="webppt"><a href="'.$webppt.'" class="button pdf ugdv_link" target="_blank" >Slides</a></span>&nbsp;';
    }

    if(trim($webgit) != "") {
      $ret .= ' <span class="webppt"><a href="'.$webgit.'" class="button pdf ugdv_link" target="_blank" > Github </a></span>&nbsp;';
    }
